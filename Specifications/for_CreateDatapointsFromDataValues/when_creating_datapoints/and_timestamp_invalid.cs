...
public class and_timestamp_invalid : given.a_creator
{
    ...
    Establish context = () =>
    {
        ...
        nodevalue = new NodeValue(new NodeId("ns=1;i=1"), new (){Value = 2, StatusCode = StatusCodes.Good, SourceTimestamp = new DateTime(2023, 1, 1), ServerTimestamp = new DateTime(2024, 1, 1)});
    };

    static OpcuaDatapointOutput result;
    Because of = () => result = parser.CreateDatapointFrom(nodevalue);

    It should_log_warning = () => logger.Verify(_ => _.Warning("Timestamp from Source is invalid for node {NodeId} - {DateTime}", new NodeId("ns=1;i=1"), new DateTime(2023, 1, 1)));
